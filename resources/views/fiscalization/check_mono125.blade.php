          ...
          var checkPaymentInrerval = setInterval(function () {
            ...
            //resp.success = true;
              if (resp.success) {
                ...
                if (resp.msg == "BUSY") {
                  $('p.cntrlmsg').html('Контроллер відхилив запит на оплату.<br>Спробуйте через 30 сек');
                ...
                {
                  clearInterval(checkPaymentInrerval);
                  $('p.cntrlmsg').html('Не дочекались відповіді від контроллера.<br>Спробуйте через 20 сек');
                  $('p.cntrlmsg').css('color', 'red');
